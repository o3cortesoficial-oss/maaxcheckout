...
const defaultStorefront = {
  ...
  intro_left: "Design que chama atenção",
  intro_right: "Uma marca inesquecível",
  statement_title: "Descubra o que torna cada escolha especial.",
  testimonial_quote: "Uma experiência de compra direta, bonita e fácil de entender.",
  testimonial_text: "Produtos bem apresentados ajudam você a escolher com confiança e tranquilidade.",
  story_title: "Não é apenas um produto. É algo feito para marcar.",
  promise_title: "Não entregamos apenas produtos. Entregamos uma experiência extraordinária.",
...
function StorefrontPreview({ settings, products, productImages, compact = false }) {
  ...
  const editorialImage = (activeProducts[1] && coverFor(activeProducts[1].id)) || heroImage;
  const storyImage = (activeProducts[2] && coverFor(activeProducts[2].id)) || editorialImage || heroImage;
  return (
    <div
    ...
    >
      ...
      <section className="fresh-store-intro"><h2>{settings.intro_left}</h2><div>{editorialImage ? <img src={editorialImage} alt="" /> : <Package />}</div><span>{settings.intro_right}</span></section>
      ...
      <section className="fresh-store-statement"><small>POR QUE ESCOLHER</small><h2>{settings.statement_title}</h2></section>
      <section className="fresh-store-story"><article><small>O QUE DIZEM SOBRE NÓS</small><div className="fresh-story-tabs"><span>Experiência</span><span>Qualidade</span><span>Atendimento</span></div><h3>“{settings.testimonial_quote}”</h3><p>{settings.testimonial_text}</p><b>Cliente verificado</b></article><div>{editorialImage ? <img src={editorialImage} alt="Produto em destaque" /> : <Package />}<aside><small>{settings.store_name}</small><b>Programa de vantagens</b><span>Benefícios pensados para quem acompanha a marca.</span></aside></div></section>
      <section className="fresh-store-statement alternate"><small>NOSSA HISTÓRIA</small><h2>{settings.story_title}</h2></section>
      ...
      <section className="fresh-store-statement promise"><small>NOSSA PROMESSA</small><h2>{settings.promise_title}</h2></section>
...
function StorePage({ workspace, products = [], productImages = [], checkoutConfig, onReload }) {
  ...
  const sectionFields = {
    ...
    hero: <><label className="store-toggle-row"><span><b>Produto principal</b><small>Ocultar remove também todo o espaço do hero.</small></span><input type="checkbox" checked={settings.hero_visible} onChange={(e) => change("hero_visible", e.target.checked)} /></label><Field label="Chamada superior" value={settings.hero_kicker} onChange={(e) => change("hero_kicker", e.target.value)} /><Field label="Título" value={settings.hero_title} onChange={(e) => change("hero_title", e.target.value)} /><label className="data-field">Descrição<textarea rows="3" value={settings.hero_text} onChange={(e) => change("hero_text", e.target.value)} /></label><Field label="Texto do botão" value={settings.hero_button} onChange={(e) => change("hero_button", e.target.value)} /><div className="store-upload-field"><span><b>Imagem recortada do produto</b><small>Use PNG ou WebP com fundo transparente. Recomendado: 900 × 1100 px.</small></span>{settings.hero_image_url && <img className="wide" src={settings.hero_image_url} alt="Produto principal atual" />}<label>{uploading === "hero" ? "Enviando..." : settings.hero_image_url ? "Substituir imagem" : "Adicionar imagem"}<input type="file" accept="image/png,image/jpeg,image/webp" onChange={(e) => upload("hero", e.target.files?.[0])} /></label>{settings.hero_image_url && <button type="button" onClick={() => change("hero_image_url", "")}><Trash /> Remover</button>}</div></>,
    ...
    editorial: <><Field label="Título de abertura à esquerda" value={settings.intro_left} onChange={(e) => change("intro_left", e.target.value)} /><Field label="Título de abertura à direita" value={settings.intro_right} onChange={(e) => change("intro_right", e.target.value)} /><Field label="Manifesto da marca" value={settings.statement_title} onChange={(e) => change("statement_title", e.target.value)} /><label className="data-field">Depoimento<textarea rows="3" value={settings.testimonial_quote} onChange={(e) => change("testimonial_quote", e.target.value)} /></label><label className="data-field">Complemento do depoimento<textarea rows="3" value={settings.testimonial_text} onChange={(e) => change("testimonial_text", e.target.value)} /></label><Field label="História da marca" value={settings.story_title} onChange={(e) => change("story_title", e.target.value)} /><Field label="Promessa da marca" value={settings.promise_title} onChange={(e) => change("promise_title", e.target.value)} /></>,
